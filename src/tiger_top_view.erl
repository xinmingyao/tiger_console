...
%%==============================================================================
-module(tiger_top_view).

-author('mazen.harake@erlang-solutions.com').

-include("tiger_console_main.hrl").
...
fetch_and_update(State, IsForced) ->
	...
	{Time, {ok, timeout}} ->
	    State;
	{Time, {ok, Data}} ->
	    update_screen(Time, Data),
	    if not IsForced -> erlang:send_after(State#state.interval, self(), time_update);
	       true -> ok
	    end,
	    State;
	timeout ->
...
update_screen(Time,ZabDates) ->
    [ZabDate|_]=ZabDates,
    Headers=[K||{K,_}<-ZabDate],
    Yy=1,
    cecho:move(Yy,0),
    lists:foldl(fun(Header, X) -> 

			cecho:hline($ , ?MAX_HLINE),
			cecho:addstr(io_lib:format("~p ",[Header])), X + 1
		end, 1, Headers),
    Dates=lists:map(fun(A)->
		      [V||{_,V}<-A] end ,ZabDates),
    {Y, _} = cecho:getmaxyx(),
    StartY = (Y-(Y-7)),
    lists:foreach(fun(N) -> cecho:move(N, 0), cecho:hline($ , ?MAX_HLINE) end, lists:seq(StartY, Y)),

    lists:foldl(fun(Row,Y1)->
			cecho:move(Y1,0),
			lists:foldl(fun(Header, X) -> 
					    cecho:hline($ , ?MAX_HLINE),
					    cecho:addstr(io_lib:format("~p ",[Header]))
						, X + 1
				    end, 1, Row),
			Y1+1 end ,Yy+2,Dates),
